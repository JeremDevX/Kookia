...
export const getActionablePredictions = async () => (await getPredictions()).filter((prediction) => prediction.recommendation?.action === "buy" || prediction.recommendation?.action === "reduce");
