...
trois noms historiques sans toucher aux migrations. `npx prisma validate`,
`npm run build:api` et le diff final (`No difference detected`) passent ; le
conteneur jetable a été supprimé. Commit local du modèle : `53ff478`.
...
| C3 — chaîne métier complète | Prouvé localement (API / fixture synthétique) | Non applicable | `timelineEventMappers` expose décisions, commandes internes et réceptions rapprochées avec provenance simulation, source liée et coupe `asOf`; les archives ouvrent la pièce dans Achats. `demoStory.integration.test.ts` couvre quatre chapitres, 2023 sans réception source, suggestion → décision → commande démo → réception synthétique → `/impact` calculé depuis les opérations. Les décisions d’achat à instantané complet ont maintenant « Rejouer ce geste » : la route dédiée recopie le snapshot dans un bac `WorkspaceDocument` idempotent, affiche une ligne hypothétique/exclusion et ne crée aucune commande, réception ou mouvement. Test API : répétitions concurrentes, isolation inter-tenant, stock/révisions/commandes/réceptions inchangés. Chrome headless synthétique : Tab/Entrée, focus visible, sans débordement à 320/375/480/768/1280 px. | Rejeu borné aux snapshots de décisions d’achat valides, fondé sur les valeurs conservées au moment de la décision ; il n’exécute aucune opération métier dans ce bac. `verify:local-delivery` passe (18 migrations, 26 suites/39 tests, sauvegarde/restauration). Le parcours visuel complet de chaque chapitre et l’audit métier des transcriptions restent distincts ; Q2 natif/zoom 200 %/lecteur d’écran reste ouvert (`browsers: []`, `cgWindowNotFound`). Aucune précision terrain ni aucun KPI seedé ; aucun envoi n'est déclenché. |
...
| 2026-09-25 | Audit schéma/migrations | `53ff478` | PostgreSQL 16 tmpfs neuf, migrations 18/18, puis `prisma migrate diff --from-url <base-jetable> --to-schema-datamodel prisma/schema.prisma --exit-code` : ajout au modèle des index déjà créés par les migrations (`StockMovement`/`SaleContribution`) et `map` pour trois noms PostgreSQL tronqués. Revalidation : aucun diff. `npx prisma validate`, `npm run build:api`, `git diff --check`. | Aucun SQL de migration modifié ; le conteneur jetable a été supprimé. | Reprendre le premier manque local non bloqué dans la matrice. |
