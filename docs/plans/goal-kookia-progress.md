...
le ledger fixture est déterministe et chaque mois de service de mai 2023 à
septembre 2026 est couvert. Après autorisation de l'accès au moteur local,
`npm run verify:local-delivery` passe également : PostgreSQL 16 tmpfs neuf,
18 migrations et parité Prisma, mêmes contrôles de code, 26 fichiers/41 tests
d'intégration et sauvegarde/restauration synthétique. Le conteneur exact créé
par la recette est absent après nettoyage. Aucun compte conservé n'a été testé
ni modifié. CUA liste toujours `browsers: []` et `getApp("Google Chrome")`
retourne `cgWindowNotFound` ; la revue native 200 %/technologie d'assistance
reste ouverte. Un runner `demo:fixtures` temporaire a répondu HTTP 200 sur
`/login` et `/api/health`, mais macOS n'a pas pu ouvrir le bundle Chrome
(`kLSNoExecutableErr`) ; aucune capture native n'a été obtenue. Le runner exact
a été interrompu et son conteneur, ses ports et son fichier d'accès temporaire
ont été vérifiés absents ; les anciens bacs n'ont pas été touchés. Le Goal
reste en cours.
...
| --- | --- | --- | --- | --- |
| Q1 — garde et base d'intégration isolée | Prouvé localement | Non applicable | Garde URL fail-closed ; revalidation R0 fraîche : migrations 18/18, diff Prisma vide, lint, builds web/API, 153 tests Vitest, 33 tests Node/CSS, 41 tests d'intégration (26 fichiers) et restauration synthétique passent. Conteneur PostgreSQL tmpfs supprimé. | Workflow CI ajouté mais non exécuté sur GitHub ; aucune base conservée utilisée. Deux tentatives fraîches antérieures ont eu des échecs d'intégration variables, non reproduits à la dernière recette complète. |
| Q1b — bac de scénario jetable | Prouvé localement (fixture seulement) | Non applicable | Fixture synthétique 431 pièces ; plan quatre ans déterministe ; test PostgreSQL crée l'archive et le ledger dans un tenant, une pièce sans mouvement dans un second, et rejoue une réception avec delta unique dans un troisième ; vérifie isolation et suppression en cascade. `npm run demo:fixtures` amorce aussi une base tmpfs pour la revue UI sans lire les transcriptions locales ; preuve GET réelle plus bas. Le signal SIGINT livré directement au processus Node du runner sort en 130 et supprime conteneur tmpfs, serveurs et identifiants temporaires. | Le rejeu est un helper de fixture, pas une garantie de C1 en production. La revue visuelle native reste indisponible (CUA). CI GitHub non déclenchée ; exclusion sparse-checkout configurée mais non observée à distance. |
...
| C3 — chaîne métier complète | Prouvé localement (fixtures isolées, suites complémentaires) | Non activé — données d’activité mesurée absentes | `demoStory.integration.test.ts` relie les chapitres 2023–2026, pièces et provenance, décision, commande et réception simulées, impact puis Historique en lecture seule ; vérifie l’absence de mouvement réel à la réception simulée, l’exclusion du reçu simulé des estimations enregistrées et la coupe « Connu au ». `ingredientOutflowEstimate.integration.test.ts` couvre les réceptions positives enregistrées, les recettes datées et unités compatibles, l’estimation 90/10, l’entrée non couverte puis la recette revue, les bornes de dates et l’isolation restaurant, sans écriture de ventes, pertes, production ou stock. `restaurantSimulationPlan.test.ts` couvre chaque mois de service de mai 2023 à septembre 2026 et le rejeu déterministe. La recette R0 courante passe : 26 fichiers/41 tests d’intégration, 39 fichiers/153 tests Vitest, 33 contrôles Node/CSS, migrations/parité et sauvegarde/restauration. | Preuves techniques sur fixtures uniquement ; aucune sortie terrain n’est déclarée ni injectée au compte KookIA. Les estimations restent distinctes des opérations observées et des KPI mesurés. Le zoom natif et le lecteur d’écran restent ouverts dans la matrice Q2. |
...
| I3 — Ticket Z candidat | Prouvé localement (transcription manuelle, sans OCR) | Non activé (aucun OCR/fournisseur configuré) | Upload PDF/JPEG/PNG borné à 4 Mio ; signature, dimensions d'image et métadonnées validées. Les octets ne sont pas persistés ni envoyés ; hash et transcription le sont. Une contribution attend revue, exige un article et ne devient vente qu'après décision ; ligne sans détail = aucune vente. Test API couvre auth/deux tenants, hash/rejeu, date ISO/FR, signatures/limites, suppression brouillon, conflits CSV/POS, projection KPI et absence de pièce brute. | PostgreSQL 16 tmpfs jetable sans volume : migration fraîche 18/18 ; intégration 22 fichiers/32 tests ; `npm test` 20 Vitest/90 + 30 Node/CSS ; lint, builds web/API, Prisma et diff-check. Chrome headless/CDP sur nouveau `demo:fixtures` tmpfs : état upload et erreur de type visibles à 320 px, formulaire à 320/768/1280 px sans débordement horizontal, noms AX des champs et boutons, Tab vers le champ fichier avec focus visible, ajout/soumission candidate au clavier ; le GET des ventes est identique avant/après, sans vente pré-revue. Captures ci-dessous. Aucun OCR réel ; nombre de pages PDF/décodage complet non contrôlés. Hash/transcription conservés jusqu'à suppression tenant, sans TTL ; aucun lecteur d'écran réel ni zoom natif vérifié (CUA indisponible). |
...
| 2026-09-26 | Q1/R0 — revalidation intégrale | `8c7252f` (état vérifié) | Deux recettes fraîches ont échoué sur des tests d'intégration différents ; test achat ciblé et suite complète ont ensuite passé → recette R0 complète repassée sur un troisième bac frais. | `npm run verify:local-delivery` : migrations 18/18, parité Prisma, lint, builds web/API, `npm test` (39 fichiers/153 Vitest + 33 contrôles Node/CSS), `test:integration` (26 fichiers/41 tests), sauvegarde/restauration et statut Prisma passent. Les erreurs précédentes étaient `socket hang up` dans achat et 500 dans correction de vente ; cause non établie, donc intermittence non réputée éliminée. Les conteneurs exacts étaient tmpfs sans volume. | Surveiller une récurrence sans masquer les assertions ; poursuivre les états Q2 encore ouverts, lecteur d'écran et zoom natif restant non vérifiés. |
...
### R0 — disponibilité PostgreSQL distinguée du liveness (2026-09-26)

`GET /api/health` demeure un contrôle de présence du processus. Le nouvel
`GET /api/ready` lance `SELECT 1` via Prisma : réponse `ready` si la base répond,
HTTP 503 `not_ready` si le test échoue, sans exposer l'erreur ou les détails de
connexion. L'intégration couvre la base prête, la panne simulée et le liveness
toujours disponible. La documentation de livraison locale et de démarrage
décrit les deux contrats ; readiness ne prétend vérifier ni migrations ni
requêtes métier.

`npm run verify:local-delivery` passe sur PostgreSQL 16 tmpfs : lint, build
web/API, 18 migrations fraîches, parité Prisma, 124 tests unitaires, 26 fichiers
et 41 tests d'intégration, dump/restore avec témoin synthétique. Le conteneur
sans volume est supprimé en fin de recette. Aucun compte ou jeu de données
...
### Audit documentaire de R1 (2026-09-26)

L'audit en lecture seule du dépôt confirme une préparation locale R0, pas une
préproduction : `vercel.json` ne sert que le fallback SPA, le seul workflow
versionné est `.github/workflows/ci.yml` et il ne contient pas de commande de
déploiement ; `docs/local-delivery.md` décrit explicitement les prérequis
manquants d'hébergement, routage/API, TLS/origine, secrets et sauvegardes
persistantes. La ligne R1 distingue maintenant cet état local de l'état
d'éventuels environnements externes, qui n'ont pas été consultés. Aucun
déploiement, appel externe, push ou lecture de credential n'a été effectué.
R1 reste non activé et distinct de la portée locale obligatoire. La revue
Q2 UI/KPI reste en cours ; CUA retourne `browsers: []` et `getApp("Google
Chrome")` échoue toujours avec `cgWindowNotFound`.

### I3 — revue rendue du Ticket Z (2026-09-26)

Sur un nouveau compte `demo:fixtures` et PostgreSQL tmpfs, sans corpus local ni
compte persistant, Chrome headless/CDP a rendu `/sales` et le vrai composant
`SalesTicketImport`. Un fichier non accepté affiche l'erreur ; un PDF de fixture
ouvre l'aperçu local et le formulaire. À 320/768/1280 px, document et corps
restent à la largeur du viewport (320/768/1280 px) ; le panneau mesure
288/722/928 px sans débordement. Au clavier, Tab depuis le titre atteint le
champ fichier avec contour visible ; Entrée ajoute une ligne puis soumet une
candidate. L'AXTree expose les noms du champ Article, de la quantité, du jour de
service, du bouton Ajouter une ligne et du bouton de soumission. Un GET des
ventes avant/après confirme qu'aucune vente enregistrée ne change avant la
revue ; seules les requêtes mutatrices de cette passe sont login, upload de la
pièce et enregistrement de la candidate.

Captures QA techniques inspectées : [erreur 320](evidence/q2-ticket-z/ticket-z-error-320.png),
[aperçu 320](evidence/q2-ticket-z/ticket-z-upload-320.png),
[formulaire 768](evidence/q2-ticket-z/ticket-z-upload-768.png),
[formulaire 1280](evidence/q2-ticket-z/ticket-z-upload-1280.png),
[formulaire après défilement 320](evidence/q2-ticket-z/ticket-z-form-320.png),
[ligne candidate après défilement 320](evidence/q2-ticket-z/ticket-z-line-320.png) et
[candidate enregistrée 320](evidence/q2-ticket-z/ticket-z-candidate-320.png). Après
défilement à 320 px, le formulaire et le fieldset de ligne mesurent 238 px, sans
débordement horizontal ; les captures montrent les champs d'article et de quantité.
La candidate de QA n'existe que dans le runner isolé et n'a aucune valeur métier.
Les touches utilisées pour la vérification sont injectées par CDP, pas un clavier
natif ; CUA refuse toujours le navigateur Chrome.

Le `Ctrl-C` synthétique envoyé au PTY par l'outil d'exécution a arrêté le groupe
sans déclencher le gestionnaire SIGINT de Node (un probe Node minimal sort aussi
en 1 sans journal de gestionnaire) ; le conteneur et les identifiants de ce run
précis ont été supprimés manuellement. Un second run, avec SIGINT ciblé sur le
PID Node exact, termine en 130 et exécute son `finally` : conteneur du run,
ports 60080/60081 et répertoire d'identifiants sont absents. Les anciens bacs
Docker n'ont pas été modifiés. Cela vérifie le chemin de nettoyage du runner,
sans revendiquer le contrôle CUA natif ni une revue par lecteur d'écran. Un
dernier run de capture arrêté par SIGINT ciblé sur le PID Node exact termine lui
