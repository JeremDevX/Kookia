...
| 2026-09-25 | Q1b/C3 — runner UI sans corpus | `bd8d60f` | Ajout de `npm run demo:fixtures`, qui amorce les fixtures synthétiques existantes dans le PostgreSQL tmpfs du runner ; `demo:local` reste inchangé. Un compte, 437 pièces/sources, 946 journées de service et 5 026 ventes simulées. | API et page `/login` répondent 200 ; GET authentifiés juin 2023/24/25/26 : 1 366/1 347/1 305/1 329 événements, dont 1 014/1 014/988/985 événements de provenance simulation ; Bilan GET : 45 mois, aucune donnée enregistrée, 100 431 unités simulées, 5 026 ventes simulées exclues. Lint, build web, 29 fichiers/119 tests Vitest + 33 contrôles Node/CSS, diff-check passent. Arrêt confirmé : conteneur propre, ports 57807–57809 fermés, fichier de mot de passe temporaire supprimé. | Aucune transcription locale lue. CUA `browsers: []`/`cgWindowNotFound` persiste, donc pas de capture ni de revue rendue sur ce runner ; les preuves visuelles antérieures restent headless/CDP. | Réutiliser ce runner pour compléter seulement les parcours UI qui exigent un backend réel ; garder l'écran natif/lecteur d'écran comme limite ouverte. |
