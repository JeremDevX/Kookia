...
| C3 — chaîne métier complète | Partiellement prouvé (fixtures QA uniquement) | Non activé — sorties terrain absentes | Les tests C3 antérieurs vérifient le câblage technique de sources, décisions, commandes, réceptions et calculs d'impact sur des fixtures isolées. Le parcours Historique reste en lecture seule ; le CTA, la page et l'API de rejeu sont retirés dans cette tranche. Les captures Histoire existantes, dont le commit local `87a0867`, restent des preuves de clavier/responsive uniquement et non des données restaurant. | Les transcriptions d'achat ne fournissent pas les ventes/services/productions/pertes nécessaires pour prouver cette chaîne sur le compte Kookia. Ne rien inventer dans le ledger ou les KPI ; l'absence de données de sortie reste inconnue. L'inspection native du compte demeure à faire sur l'URL configurée si une fenêtre contrôlable est disponible. |
