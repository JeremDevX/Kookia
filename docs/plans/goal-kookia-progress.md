...
d’observer le chargement. Le rendu initial désactivait ce bouton et perdait le
focus ; Tab était alors récupéré sur le bouton de fermeture supérieur. Le
correctif conserve le bouton dans l’ordre de tabulation pendant le calcul,
l’annonce `aria-disabled=true` et garde le garde anti-double-soumission. Après
la réponse, Tab atteint l’idée faisable et Entrée l’applique au brouillon. Le
piège Shift+Tab/Échap boucle et rend le focus au déclencheur. L’AXTree expose
