...
| C1 — facture source actionnable | Q1b + réception manuelle actuelle | Partir des **431 fiches versionnées** (ne pas supposer leur présence dans la base isolée), non d'un futur OCR : rechercher une pièce, montrer date source/date décalée (ou origine incertaine), statut et lignes candidates, préremplir un brouillon corrigible, associer fournisseur/produit/unité/prix, puis laisser **réceptionner explicitement** par le flux transactionnel existant. Persister côté serveur l'identité de pièce `(restaurant, sourceDocumentId)` et ses révisions/hash entre archive, brouillon, mouvements de démonstration, commande et livraison ; contrainte/idempotence empêchent deux factures ou clics concurrents de créditer la même pièce. Mesurer couverture/rejets ; avoir, BL et pièce illisible ne créditent rien sans revue. Refuser explicitement une réception partielle non prise en charge jusqu'à O2. Tester conflit, autre restaurant et pièce déjà simulée. |
| C2 — récit de quatre années civiles | C1 + D2 à D6 | Étendre le scénario **2023–2026 décalé** en chronologie continue consultable par période/service : pièces, stocks, recettes versionnées, productions, ventes, pertes et états partiels ; distinguer fait source, hypothèse, simulation et inconnu. En 2023, aucune réception sourcée : pièce consultée seulement, ouverture/réappro fictif séparé. Générateur déterministe, idempotent et réversible sur espace isolé ; aucune écriture de test sur Camille. Ce lot ne prétend pas livrer les futurs calculs d'achat ou d'impact. Voir [le mandat de scénario](goal-kookia-reference.md). |
...
| I4 — extraction d'une nouvelle facture | C1 + politique de stockage/accès | Ajouter OCR seulement si le flux C1 sait déjà traiter un candidat : contrôler fichier, extraction, comparaison à l'original et correction humaine. Une extraction n'est ni une réception ni un crédit de stock. Tester même pièce rejouée, correction et accès autre restaurant. Le corpus transcrit existant ne dépend pas de cet OCR. |
...
| C3 — chaîne démonstrative complète | C2 + F4 + O2 + M1 | Relier le récit continu aux suggestions d'achat, décisions, commandes, réceptions et bilans réellement calculés depuis les opérations du **tenant démo**, tous étiquetés simulation, sans KPI seedés ni envoi. Rejouer quatre chapitres et transitions avec pièce/source et provenance à chaque geste, y compris 2023 sans réception de pièce. Le tenant réel continue de refuser un achat opérationnel basé sur `demo_simulation`. Ce lot est la porte d'acceptation de la démonstration, pas une nouvelle source de vérité. |
