...
Seules les lignes comportant une quantité explicite et un prix unitaire ou un montant de ligne exploitable génèrent un produit et une entrée de stock de **démonstration** ; les pièces sans ces données restent consultables. Lorsque seul le montant de ligne est disponible, le prix unitaire de démonstration est calculé en le divisant par la quantité. Les produits sont classés par famille métier à partir du libellé et, si elle est suffisamment spécialisée, de la provenance fournisseur. Un libellé illisible ou équivoque reste « À identifier » : aucune catégorie générique « Factures importées » n’est utilisée. Les avoirs et bons de livraison ne créent pas d’entrée. Des sorties `simulated_consumption` sont ajoutées entre les achats et après la dernière livraison pour illustrer le fonctionnement du stock : ni ces sorties ni les entrées OCR ne constituent une réception ou une consommation réelle validée. Les dates décalées ne sont pas des dates comptables.
