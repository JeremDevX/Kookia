...
reportRoutes.get("/report", async (req, res, next) => {
  try {
    ...
    const [analytics, predictions, movements, productions, orders] = await prisma.$transaction([
      ...
      prisma.prediction.findMany({ where: { restaurantId, predictedDate: { gte: start, lt: end } }, include: { product: { select: { name: true, unit: true } } }, orderBy: { predictedDate: "asc" } }),
    ...
    for (const item of predictions) {
      rows.push({ section: "Prévisions", date: item.predictedDate.toISOString().slice(0, 10), metric: `${item.product.name} — consommation prévue (${item.product.unit})`, value: Number(item.predictedConsumption), source: item.source });
      rows.push({ section: "Prévisions", date: item.predictedDate.toISOString().slice(0, 10), metric: `${item.product.name} — confiance (0–1)`, value: item.confidence, source: item.source });
    }
