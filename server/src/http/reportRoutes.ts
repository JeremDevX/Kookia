...
reportRoutes.get("/report", async (req, res, next) => {
  try {
    ...
    const rows: Row[] = [];
    for (const item of sales) rows.push({ section: item.source === "demo_simulation" ? "Ventes simulées" : "Ventes enregistrées", date: item.serviceDate.toISOString().slice(0, 10), metric: `${item.saleItem.name} — unités vendues`, value: item.quantity, source: item.source === "demo_simulation" ? "Simulation de démonstration" : item.source === "csv" ? item.revision ? "Import CSV corrigé" : "Import CSV" : "Saisie manuelle" });
    for (const item of movements) rows.push({ section: "Mouvements de stock", date: item.createdAt.toISOString(), metric: `${item.product.name} — ${item.reason} (${item.product.unit})`, value: Number(item.delta), source: item.operationId.startsWith("restaurant-simulation-v1:") ? "Simulation de démonstration" : "Opération enregistrée" });
    for (const item of productions) rows.push({ section: "Productions et refus", date: item.date.toISOString().slice(0, 10), metric: `${item.recipeName} — ${item.kind}`, value: item.portions, source: item.notes.includes("restaurant-simulation-v1") ? "Production simulée — portions estimées" : "Déclaration enregistrée — portions" });
