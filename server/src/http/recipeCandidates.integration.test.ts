...
function candidate(name: string, productId: string, sourceDocumentId: string, unit: string) {
  return { name, category: "Plat", prepTime: 20, yieldPortions: 4, effectiveFrom: todayParis(),
    ingredients: [{ productId, quantity: unit === "L" ? 0.4 : 0.6, sourceDocumentId, sourceLineNumber: 4 }] };
}
...
it("keeps recipe candidates hypothetical until an atomic, stock-neutral confirmation", async () => {
  ...
  const mushrooms = await product(demo.agent, demo.supplierId, "Champignons fixture", "kg");
  const cream = await product(demo.agent, demo.supplierId, "Crème fixture", "L");
  const mushroomSource = await sourceInvoice(demo.restaurantId, "Champignons", "kg");
  const creamSource = await sourceInvoice(demo.restaurantId, "Crème", "L");
  ...
  expect(first.body).toMatchObject({ status: "pending", revision: 1, recipeId: null,
    ingredients: [{ productId: mushrooms.id, evidence: { sourceDocumentId: mushroomSource.id,
      sourceDocumentRevision: 0, sourceLineNumber: 4, sourceName: "Champignons" } }] });
