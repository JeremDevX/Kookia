...
export async function listTimeline(restaurantId: string, from: string, to: string, asOf: string) {
  ...
  const events: TimelineEvent[] = [
    ...
    ...sales.map((sale): TimelineEvent => ({
      ...
      label: sale.source === "demo_simulation" ? "Vente à vérifier" : "Vente enregistrée",
      detail: `${safeText(sale.saleItem.name)} · ${sale.quantity} article(s)`,
      provenance: sale.source === "demo_simulation" ? "simulation" : "recorded",
      ...(sale.source === "demo_simulation" ? { qualifier: "Cette vente est hors bilan et n’est pas retenue dans les ventes enregistrées." } : {}),
      href: "/sales",
    })),
    ...serviceDays.map((day): TimelineEvent => ({
      ...
      detail: `Couverture ${day.coverage} · origine ${day.source === "demo_simulation" ? "hors bilan" : day.source}`,
      ...
      ...(day.coverage !== "complete" ? { qualifier: "État partiel ou manquant ; les ventes disponibles ne décrivent pas un service complet." }
        : day.source === "demo_simulation" ? { qualifier: "Cet état de service est hors bilan et ne complète pas le calendrier enregistré." } : {}),
      href: "/sales",
    })),
