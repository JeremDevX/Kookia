...
export interface OrderInput {
  operationId: string;
  lines: { productId: string; quantity: number; predictionId?: string }[];
}
  ...
  return prisma.$transaction(async (tx) => {
    ...
    if (prior) {
      ...
      if (!Array.isArray(previous) || previous.length !== input.lines.length || !previous.every((line, index) => {
        ...
        return line.productId === current.productId && line.quantity === current.quantity && (line.predictionId ?? null) === (current.predictionId ?? null);
